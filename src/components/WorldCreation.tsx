...
export default function WorldCreation() {
  ...
  const setNpcLites = (val: typeof npcLites) => updateWorldCreation({ npcLites: val });
...
\`\`\`json
{
  ...
  "npcLites": ${JSON.stringify(npcLites)},
...
Dữ liệu trả về PHẢI là một object JSON duy nhất với cấu trúc chính xác sau:
{
  ...
  "npcLites": [{
    ...
    "age": "Tuổi",
    "gender": "Giới tính",
    "description": "Mô tả ngắn gọn (ngoại hình, vai trò mờ nhạt)"
  }],
  "worldDetails": { "places": "Mô tả chi tiết các địa điểm quan trọng hoặc những nơi các nhân vật sẽ xuất hiện hoặc đi đến bước đầu" }
}

LƯU Ý QUAN TRỌNG: NẾU NGƯỜI CHƠI CHỈ NHẬP Ý TƯỞNG SƠ KHAI MÀ KHÔNG NHẬP CÁC THÔNG TIN KHÁC, BẠN PHẢI TỰ ĐỘNG SÁNG TẠO 100% (GHI ĐÈ LÊN CÁC TRƯỜNG DỮ LIỆU CŨ NẾU NÓ KHÔNG PHÙ HỢP VỚI Ý TƯỞNG MỚI NÀY, ĐẶC BIỆT LÀ CÁC NPC VÀ NPCLITE CỦA THẾ GIỚI CŨ).`;
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (data.npcLites) {
            // make sure each npcLites has an id
            const lites = Array.isArray(data.npcLites) ? data.npcLites.map((lite: any) => ({ ...lite, id: lite.id || Date.now().toString() + Math.random().toString() })) : [];
            setNpcLites(lites);
          }
...
Hãy trình bày một cách cuốn hút và logic. BẮT BUỘC TRẢ LỜI VÀ SUY NGHĨ TOÀN BỘ BẰNG TIẾNG VIỆT 100%.`;
  ...
  const handleCreate = () => {
    ...
    setGameData({
      ...
      npcLites: JSON.parse(JSON.stringify(npcLites)),
      worldDetails
    });
    setTimeout(() => {
      navigate('/gameplay');
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!data.npcLites) data.npcLites = [];
              ...
              {activeTab === 'npc' && (
                <div className="space-y-12">
                  ...
                  <div className="mt-16 space-y-6">
                    <h3 className={`text-xl font-black uppercase tracking-widest border-b pb-4 ${theme.group === 'Dark' ? 'border-white/10 text-white/50' : 'border-black/10 text-[#334155]/60'}`}>NPC PHỤ (NPCLITE)</h3>
                    <p className={`text-sm ${theme.textSecondary}`}>NPCLite là những nhân vật tối giản, có vai trò mờ nhạt hoặc mới chỉ lướt qua. Bạn có thể cho họ tiến hóa thành NPC chính trong Gameplay nếu cần.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {npcLites.map((lite, idx) => (
                        <div key={lite.id || idx} className={`p-4 rounded-xl border relative shadow-md ${theme.group === 'Dark' ? 'bg-white/5 border-white/10' : 'bg-white/80 border-amber-200'}`}>
                          <button 
                            onClick={() => setNpcLites(npcLites.filter((_, i) => i !== idx))}
                            className="absolute top-3 right-3 p-1.5 rounded-md bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all cursor-pointer z-10"
                            title="Xóa NPCLite"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                          <div className="space-y-3 pr-8">
                            <CharacterInput label="TÊN" value={lite.name} onChange={(val) => { const n = [...npcLites]; n[idx] = { ...n[idx], name: val }; setNpcLites(n); }} />
                            <div className="flex gap-2">
                              <div className="flex-1">
                                <CharacterInput label="TUỔI" value={lite.age} onChange={(val) => { const n = [...npcLites]; n[idx] = { ...n[idx], age: val }; setNpcLites(n); }} />
                              </div>
                              <div className="flex-1">
                                <CharacterInput label="GIỚI TÍNH" value={lite.gender} onChange={(val) => { const n = [...npcLites]; n[idx] = { ...n[idx], gender: val }; setNpcLites(n); }} />
                              </div>
                            </div>
                            <CharacterTextArea label="MÔ TẢ NGẮN" value={lite.description} onChange={(val) => { const n = [...npcLites]; n[idx] = { ...n[idx], description: val }; setNpcLites(n); }} rows={2} />
                          </div>
                        </div>
                      ))}
                    </div>
                    <button 
                      onClick={() => setNpcLites([...npcLites, { id: Date.now().toString() + Math.random().toString(), name: '', age: '', gender: '', description: '' }])}
                      className="w-full py-4 rounded-xl border-2 border-dashed border-white/10 hover:border-white/30 hover:bg-white/5 transition-all flex items-center justify-center gap-2 text-white/50 hover:text-white cursor-pointer group"
                    >
                      <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" /> 
                      <span className="font-bold uppercase tracking-widest text-xs">Thêm NPCLite mới</span>
                    </button>
                  </div>
                </div>
              )}
