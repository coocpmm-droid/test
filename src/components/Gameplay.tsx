...
const formatNpcLitesCodex = (npcLites: any[]) => {
  if (!npcLites || !npcLites.length) return "Không có NPC Phụ nào.";
  return npcLites
    .map((lite, idx) => {
      const lines = [`NPC PHỤ ${idx + 1}:`];
      for (const [key, value] of Object.entries(lite)) {
...
export default function Gameplay() {
  ...
  const [selectedNPCLiteIndex, setSelectedNPCLiteIndex] = useState<number | null>(null);
  const [isEditingNPCLite, setIsEditingNPCLite] = useState(false);
  ...
  const generateTurn = async (userAction: string | null) => {
    ...
    let evolveInstruction = "";
    const pendingEvolutions = (gameData.npcLites || []).filter((n: any) => n.isPendingEvolve);
    if (pendingEvolutions.length > 0) {
      const evolveNames = pendingEvolutions.map((n: any) => n.name).join(", ");
      evolveInstruction = `\n\n[LỆNH TỪ HỆ THỐNG]: NGƯỜI CHƠI ĐÃ YÊU CẦU TIẾN HÓA CÁC NPCLITE SAU THÀNH NPC CHÍNH QUAN TRỌNG NGAY TRONG LƯỢT NÀY: ${evolveNames}. AI BẮT BUỘC PHẢI KHỞI TẠO ĐẦY ĐỦ THÔNG TIN CHO HỌ TRONG MẢNG \`newNPCs\` VÀ ĐỒNG THỜI GHI TÊN NHỮNG NPCLITE NÀY VÀO MẢNG \`evolvedNpcLites\` TRONG PHẦN JSON TRẢ VỀ.\n`;
    }
...
DANH SÁCH NPCLITE (NPC PHỤ):
${formatNpcLitesCodex(gameData.npcLites || [])}
...
[DANH SÁCH NPCLITE (NPC PHỤ - ĐƯỢC QUYỀN TIẾN HÓA THÀNH NPC BÌNH THƯỜNG BỞI NGƯỜI CHƠI)]
${formatNpcLitesCodex(gameData.npcLites || [])}
    ...
    try {
      ...
      if (parsedData) {
        ...
        if (currentState.gameData) {
          ...
          // Handle NPCLites
          if (parsedData.newNpcLites && Array.isArray(parsedData.newNpcLites)) {
            if (!newData.npcLites) newData.npcLites = [];
            parsedData.newNpcLites.forEach((lite: any) => {
              if (lite.name) {
                const exist = newData.npcLites.some((n: any) => n.name === lite.name);
                if (!exist) {
                  newData.npcLites.push({
          ...
          // Handle Evolved NPCLites
          if (parsedData.evolvedNpcLites && Array.isArray(parsedData.evolvedNpcLites)) {
            if (newData.npcLites) {
              const originalLength = newData.npcLites.length;
              newData.npcLites = newData.npcLites.filter(
                (lite: any) => !parsedData.evolvedNpcLites.includes(lite.name)
              );
              if (newData.npcLites.length !== originalLength) {
                hasUpdate = true;
                toast.success(`Một số NPC đã tiến hóa và lộ diện!`);
              }
            }
          }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-transparent relative overflow-hidden">
          ...
          {leftOpen && (
            ...
            >
              ...
              >
                ...
                >
                  NPCs ({(gameData.npcs?.length || 0) + (gameData.npcLites?.length || 0)})
                </h3>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                {[
                  ...(gameData.npcs || []).map((npc: any, index: number) => ({ npc, index, isLite: false })),
                  ...(gameData.npcLites || []).map((lite: any, index: number) => ({ npc: lite, index, isLite: true }))
                ]
                  .map(({ npc, index, isLite }) => {
                    ...
                    const locStr = currentNpcLoc || (isLite ? npc.location : (npc.location || ""));
                    const isUnknownLoc =
                      !locStr ||
                  ...
                  .map(({ npc, index, isLite, locStr, isUnknownLoc, isMale }, renderIndex) => {
                    ...
                    return (
                      <div
                        key={isLite ? `lite-${npc.id || index}` : `npc-${index}`}
                        onClick={() => {
                          if (isLite) {
                            setSelectedNPCLiteIndex(index);
                          } else {
                        ...
                        <div className="flex items-start gap-3">
                          {npc.avatar ? (
                            ...
                            >
                              ...
                              {isLite && (
                                <div className="absolute inset-x-0 bottom-0 py-0.5 bg-black/20 flex justify-center items-center backdrop-blur-sm">
                                  <span className={`text-[8px] uppercase font-bold tracking-wider ${textClass}`}>Phụ</span>
                                </div>
                              )}
                            </div>
                          )}
                          <div className="flex flex-col gap-1 items-start flex-1 min-w-0">
                            ...
                            >
                              ...
                              {isLite && (
                                <span className={`shrink-0 border px-1.5 py-0.5 rounded text-[8px] font-bold leading-none uppercase tracking-wider mt-0.5 ${theme.group === "Dark" ? "bg-white/10 text-white/50 border-white/10" : "bg-black/5 text-[#334155]/70 border-black/10"}`}>Phụ</span>
                              )}
                            </h4>
                            <div
                ...
                {(!gameData.npcs || gameData.npcs.length === 0) && (!gameData.npcLites || gameData.npcLites.length === 0) && (
      ...
      <AnimatePresence>
        ...
        {selectedNPCLiteIndex !== null && gameData.npcLites?.[selectedNPCLiteIndex] && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`fixed inset-0 z-[60] backdrop-blur-md flex items-center justify-center p-4 ${theme.group === "Dark" ? "bg-black/60" : "bg-black/20"}`}
            onClick={() => {
              setSelectedNPCLiteIndex(null);
              setIsEditingNPCLite(false);
            }}
          >
            <motion.div
              ...
              className={`w-full max-w-sm rounded-[2rem] p-6 shadow-2xl border flex flex-col max-h-[80vh] overflow-hidden ${theme.group === "Dark" ? "theme-panel border-white/10" : "bg-white border-black/5"}`}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-start mb-6 shrink-0 z-10 w-full relative">
                {isEditingNPCLite ? (
                  <input
                    value={gameData.npcLites[selectedNPCLiteIndex].name}
                    onChange={(e) => {
                      const newNpcLites = [...gameData.npcLites];
                      newNpcLites[selectedNPCLiteIndex] = { ...newNpcLites[selectedNPCLiteIndex], name: e.target.value };
                      setGameData({ ...gameData, npcLites: newNpcLites });
                    }}
                    className={`text-lg font-bold w-full bg-transparent border-b border-blue-500 outline-none flex-1 mr-4 ${theme.textPrimary}`}
                  />
                ) : (
                  <h3 className={`text-xl font-bold flex-1 mr-4 ${theme.textPrimary}`}>{gameData.npcLites[selectedNPCLiteIndex].name}</h3>
                )}

                <div className="flex gap-2 isolate relative z-20 shadow-sm shrink-0">
                  {isEditingNPCLite ? (
                    <button onClick={() => setIsEditingNPCLite(false)} className={`px-3 py-1.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs uppercase cursor-pointer`}>Lưu</button>
                  ) : (
                    <>
                      <button onClick={() => {
                        const npcLite = gameData.npcLites[selectedNPCLiteIndex];
                        if (npcLite.isPendingEvolve) return;
                        const newNpcLites = [...gameData.npcLites];
                        newNpcLites[selectedNPCLiteIndex] = { ...newNpcLites[selectedNPCLiteIndex], isPendingEvolve: true };
                        setGameData({ ...gameData, npcLites: newNpcLites });
                        toast.success("Đã ghi nhận yêu cầu tiến hóa! AI sẽ xử lý ở lượt kế tiếp.");
                      }} className={`px-2 py-1.5 rounded-full font-bold text-xs uppercase border flex items-center gap-1 transition-colors cursor-pointer ${
                          gameData.npcLites[selectedNPCLiteIndex].isPendingEvolve 
                            ? (theme.group === 'Dark' ? 'bg-green-500/20 text-green-400 border-green-500/50' : 'bg-green-500/20 text-green-600 border-green-500/50')
                            : (theme.group === 'Dark' ? 'hover:bg-yellow-500/20 text-yellow-400 border-yellow-500/30' : 'hover:bg-yellow-500/20 text-yellow-600 border-yellow-500/30')
                        }`} title="Yêu cầu AI Tiến hóa">
                        <Sparkles size={14} /> {gameData.npcLites[selectedNPCLiteIndex].isPendingEvolve ? "ĐANG TIẾN HÓA..." : "TIẾN HÓA"}
                      </button>
                      <button onClick={() => setIsEditingNPCLite(true)} className={`px-3 py-1.5 rounded-full hover:bg-black/10 dark-theme:hover:bg-white/10 ${theme.textSecondary} text-xs font-bold uppercase transition-colors cursor-pointer border ${theme.group === 'Dark' ? 'border-white/10' : 'border-black/10'}`}>Sửa</button>
                    </>
                  )}
                  
                  <button onClick={() => {
                      const newNpcLites = gameData.npcLites.filter((_: any, idx: number) => idx !== selectedNPCLiteIndex);
                      setGameData({ ...gameData, npcLites: newNpcLites });
                      setSelectedNPCLiteIndex(null);
                      setIsEditingNPCLite(false);
                    }} 
                    className={`p-1.5 rounded-full hover:bg-red-500/20 text-red-500 cursor-pointer border border-transparent transition-colors`} title="Xóa"
                  >
                    <Trash2 size={16} />
                  </button>
                  <button onClick={() => {
                    setSelectedNPCLiteIndex(null);
                    setIsEditingNPCLite(false);
                  }} className={`p-1.5 rounded-full hover:bg-black/10 dark-theme:hover:bg-white/10 ${theme.textSecondary} cursor-pointer transition-colors border border-transparent`}>
                    <X size={16} />
                  </button>
                </div>
              </div>

              <div className={`space-y-4 text-sm flex-1 overflow-y-auto no-scrollbar pb-2 pt-1 ${theme.textSecondary}`}>
                <div className="flex justify-between items-center border-b border-black/5 dark-theme:border-white/5 pb-2">
                  <span className="font-bold opacity-70 shrink-0 mr-4">Tuổi:</span>
                  {isEditingNPCLite ? (
                    <input
                      value={gameData.npcLites[selectedNPCLiteIndex].age}
                      onChange={(e) => {
                        const newNpcLites = [...gameData.npcLites];
                        newNpcLites[selectedNPCLiteIndex] = { ...newNpcLites[selectedNPCLiteIndex], age: e.target.value };
                        setGameData({ ...gameData, npcLites: newNpcLites });
                      }}
                      className="bg-black/5 dark-theme:bg-white/5 px-2 py-1 rounded text-right flex-1 min-w-0"
                    />
                  ) : <span>{gameData.npcLites[selectedNPCLiteIndex].age}</span>}
                </div>

                <div className="flex justify-between items-center border-b border-black/5 dark-theme:border-white/5 pb-2">
                  <span className="font-bold opacity-70 shrink-0 mr-4">Giới tính:</span>
                  {isEditingNPCLite ? (
                    <input
                      value={gameData.npcLites[selectedNPCLiteIndex].gender}
                      onChange={(e) => {
                        const newNpcLites = [...gameData.npcLites];
                        newNpcLites[selectedNPCLiteIndex] = { ...newNpcLites[selectedNPCLiteIndex], gender: e.target.value };
                        setGameData({ ...gameData, npcLites: newNpcLites });
                      }}
                      className="bg-black/5 dark-theme:bg-white/5 px-2 py-1 rounded text-right flex-1 min-w-0"
                    />
                  ) : <span>{gameData.npcLites[selectedNPCLiteIndex].gender}</span>}
                </div>

                <div className="flex justify-between items-center border-b border-black/5 dark-theme:border-white/5 pb-3">
                  <span className="font-bold opacity-70 shrink-0 mr-4 mt-1 self-start">Vị trí:</span>
                  {isEditingNPCLite ? (
                    <textarea
                      value={gameData.npcLites[selectedNPCLiteIndex].location || ""}
                      onChange={(e) => {
                        const newNpcLites = [...gameData.npcLites];
                        newNpcLites[selectedNPCLiteIndex] = { ...newNpcLites[selectedNPCLiteIndex], location: e.target.value };
                        setGameData({ ...gameData, npcLites: newNpcLites });
                      }}
                      className="bg-black/5 dark-theme:bg-white/5 px-2 py-1 rounded text-right flex-1 min-w-0 resize-none"
                      rows={2}
                    />
                  ) : (
                    <span className="text-right max-w-[60%] line-clamp-2">{latestAiMsg?.npcLocations?.find((loc: any) => loc.id === gameData.npcLites[selectedNPCLiteIndex].name)?.location || gameData.npcLites[selectedNPCLiteIndex].location || "Vị trí chưa rõ"}</span>
                  )}
                </div>

                <div className="pt-2 flex-1 flex flex-col">
                  <span className="font-bold opacity-70 block mb-2">Mô tả:</span>
                  {isEditingNPCLite ? (
                    <textarea
                      value={gameData.npcLites[selectedNPCLiteIndex].description}
                      onChange={(e) => {
                        const newNpcLites = [...gameData.npcLites];
                        newNpcLites[selectedNPCLiteIndex] = { ...newNpcLites[selectedNPCLiteIndex], description: e.target.value };
                        setGameData({ ...gameData, npcLites: newNpcLites });
                      }}
                      className="bg-black/5 dark-theme:bg-white/5 px-3 py-2 rounded flex-1 min-h-[120px] resize-none w-full"
                    />
                  ) : (
                    <p className="opacity-90 leading-relaxed whitespace-pre-wrap">{gameData.npcLites[selectedNPCLiteIndex].description}</p>
                  )}
                </div>
              </div>
            </motion.div>
